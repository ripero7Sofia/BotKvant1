			...
			if event.type == VkEventType.MESSAGE_NEW and event.to_me:
				...
				if command_words[0] == self.bot_name:
					...
					if len(command_words) > 2 and command_words[1] == "удали" and command_words[2] == "соц.сеть":
								answer = self.conn.execute(f"DELETE FROM user_info WHERE vk_id = {event.user_id}").fetchall()
								if len(answer) > 0:
									answer = answer[0][0]
									self.send_message(f'Да, конечно!Соц.сеть {answer} удалена ', event.user_id)
